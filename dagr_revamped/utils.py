...
def unlink_lockfile(lockfile):
    if not isinstance(lockfile, Path):
        raise Exception('lockfile must be a Path instance')
    if lockfile.exists():
        try:
            lockfile.unlink()
...
def http_replace(session, endpoint, dir_path, dest_subdir, dest_fname, src_subdir, src_fname):
    return http_post_json(session, endpoint, path=dir_path, dest_subdir=dest_subdir, dest_fname=dest_fname, src_subdir=src_subdir, src_fname=src_fname)
